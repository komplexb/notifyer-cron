...
async function refreshToken() {
  ...
  try {
    ...
    const deviceLoginErrors = [
      'invalid_grant',
      'interaction_required', 
      'no_tokens_found',
      'no_account_found',
      'no_cached_refresh_token',
      'refresh_token_expired'
    ];
    
    if (deviceLoginErrors.includes(error.errorCode) || 
        error.message?.includes('No refresh token found') ||
        error.message?.includes('Please sign-in')) {
      console.log(`Error requires device login: ${error.errorCode || error.message}`);
      return deviceLogin();
    }
