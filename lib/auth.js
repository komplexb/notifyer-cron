...
// Constants
const TOKEN_BUFFER_TIME_MS = 5 * 60 * 1000; // 5 minutes buffer time
const MIN_CACHE_SIZE = 10; // Minimum cache size to consider valid
  ...
  const afterCacheAccess = async cacheContext => {
    if (cacheContext.cacheHasChanged) {
      try {
        ...
        if (serializedCache && serializedCache.length > MIN_CACHE_SIZE) {
          try {
...
function deviceLogin (reason = 'Authentication required') {
  ...
  return pca
    ...
    .then(async response => {
      ...
      // Cache will be automatically persisted by afterCacheAccess callback
      return response;
    })
    .catch(error => {
...
async function refreshToken() {
  ...
  try {
    ...
    // Find matching account or use the first one
    let msalAccount = accounts[0];
    if (account && account.homeAccountId) {
      const matchingAccount = accounts.find(acc => acc.homeAccountId === account.homeAccountId);
      if (matchingAccount) {
        msalAccount = matchingAccount;
      }
    }
...
function hasValidToken () {
  ...
  const now = Date.now();
  const isValid = now <= extExpiresOn - TOKEN_BUFFER_TIME_MS;
