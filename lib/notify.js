...
async function structureMessage(note, titlePrefix, isLogin = false) {
  ...
  const isMsgTooLong = (titlePrefix + title + body.length) > 4096 + 10
 ...
 * @param {*} login - format note as device login dialog
 */
async function telegram(note, messageSettings, login = false) {
  const msg = await structureMessage(note, messageSettings.titlePrefix, login)
  ...
  const defaultRequest = {
    chat_id: messageSettings.channelHandle,
    disable_web_page_preview: messageSettings.disablePreview,
    parse_mode: 'MarkdownV2',
  }

  let text = markdownv2.bold(markdownv2.escape(msg.prefix))
  text += markdownv2.escape(`\n\n`)
  text += markdownv2.bold(markdownv2.escape(msg.title))
  text += markdownv2.escape(`\n\n`)
