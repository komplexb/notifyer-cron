...
turndownService.addRule('onenoteToMarkdown', {
  ...
  replacement: function (content, node, options) {
    ...
    const isHighlighted = style.data.includes("yellow") || style.data.includes("underline")
...
async function structureMessage(note, titlePrefix, isLogin = false) {
  ...
  console.warn("previewText", previewText)
...
async function telegram(note, messageSettings, login = false) {
  const { titlePrefix, channelHandle, showEditLink = false, disablePreview = true } = messageSettings
  ...
  text += markdownv2.escape(`\n\n`)
  text += telegramifyMarkdown(turndownService.turndown(msg.body))
