...
function getNote(settings) {
  settings = { ...settings, isSequential: false }
  return setNoteSection(settings)
    .then((section) => {
      if (settings.isSequential) {
        return getNextNote(section)
      } else {
        return p(section)
      }
    })
  ...
  function getNextNote(section) {
    return new Promise(resolve => {
      ...
      const oldCount =
        localStorage.getItem(`${settings.sectionHandle}_section_count`) || 1
      const lastPage =
        Number(localStorage.getItem(`${settings.sectionHandle}_last_page`) || "-1")
      ...
      apiRequests
        ...
        .query({
          select: 'title,links,self,id', // fields to return
          count: true, // show the amount of pages in section
          top: 1, // maximum pages query can return
          skip: nextPage
        })
        .timeout(TIMEOUTS)
        .set('Authorization', `Bearer ${accessToken}`)
