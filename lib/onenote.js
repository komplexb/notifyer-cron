 ...
 */
function getNote(settings) {
  const oldCount = Number(localStorage.getItem(`${settings.sectionHandle}_section_count`) || 1)
  return setNoteSection(settings)
    .then((section) => {
      if (settings.isSequential) {
        ...
        const nextPage = lastPage + 1 === oldCount ? 0 : lastPage + 1 // this field should be initialized to -1

        return getNextNote(section, { top: 1, skip: nextPage }) // sequential
      } else {
        const skipRange = chance.natural({
          min: 0,
          max: oldCount
        })
        return getNextNote(section, { top: 100, skip: skipRange }) // random
      }
    })
    .then(getNotePreview)
    .catch(err => {
      ...
      apiRequests
        ...
        .then(response => {
          if (response && response.ok) {
            ...
            console.warn(response.request.req.path)
            ...
            if (notes.length > 0) {
              ...
              console.log("DB", {section: settings.sectionHandle, oldCount, newCount, lastPage: nextPage})
            } else {
