...
function setNoteSection(settings) {
  ...
  return new Promise((resolve, reject) => {
    const { accessToken } = localStorage.getItem('onenote')
    const notebookQuery =
      settings.notebookName.length > 0
        ? `$filter=displayName eq '${settings.notebookName}'`
        : `$filter=isDefault eq true`
    apiRequests
      .get(`${process.env.MS_GRAPH_ROOT}/onenote/notebooks`)
      .query(notebookQuery)
      .query(`$expand=sections($select=displayName,id,pagesUrl)`)
      .timeout(TIMEOUTS)
      .set({
      ...
      .then(response => {
        if (response && response.ok) {
          const section = response.body.value[0].sections.find(
            section => section.displayName === settings.sectionName
          )
          resolve(section)
        } else {
          throw new Error(response)
