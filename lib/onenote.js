...
/**
 *
 * @param {*} section {name, id}
 * @returns promise of note object
 */
function getNoteObject(section) {
  return new Promise(resolve => {
    const { accessToken } = localStorage.getItem('onenote')
    // instead of showing the first/last 100 records,
    // randomly select a starting point and get the next 100 results
    const count =
      localStorage.getItem(`${section.name.toLowerCase()}_section_count`) || 1
    ...
    apiRequests
      .get(`${process.env.MS_GRAPH_ROOT}/onenote/sections/${section.id}/pages`)
      .query({
      ...
      .then(response => {
        if (response && response.ok) {
          ...
          localStorage.setItem(
            `${section.name.toLowerCase()}_section_count`,
            response.body['@odata.count'],
            true
          )
          // TODO: if notes is zero REJECT or RETRY
          const note = checkRecentNotes(notes, section.name)
          logRecentNote(note, section.name)
          resolve(note)
        } else {
          throw new Error(response)
        }
...
function logRecentNote(note, sectionName) {
  ...
  console.log('recent note length', recent)
